...
dag_yesterday = DAG(
    'power_data_etl_for_yesterday',
    default_args=default_args,
    description='ETL pipeline for power data for the previous day',
...
crawl_yesterday_task = PythonOperator(
    task_id='crawl_yesterday',
    python_callable=crawl_data_for_date,
    op_args=[(datetime.now() - timedelta(days=1)).strftime('%Y%m%d')],
    dag=dag_yesterday,
)

# Task dependencies
crawl_yesterday_task
